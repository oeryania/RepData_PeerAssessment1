...
Preprocess data
```{r}
#Converts interval int to time string (e.g. 5 -> "00:05") 
...
act <- mutate(act,
        date = as.Date(date),
        interval = factor(sapply(interval, to_time_str)),
...
```{r}
plot_daily_activity <- function(named_plots) {
    number_of_plots <- length(named_plots)
    par(mfrow = c(number_of_plots,1), mar = c(2, 2, 2, 2))
    for (i in 1:number_of_plots) {
        res <- summarize(named_plots[[i]], avg_steps = mean(steps, na.rm = T), .by = interval)
        plot(avg_steps ~ interval, res,
             xlab = "Interval",
             ylab = "Average Steps",
             main = names(named_plots)[i],
...
```{r}
plot_multiple <- function(named_plots) {
    number_of_plots <- length(named_plots)
    par(mfrow = c(1,number_of_plots), mar = c(0, 0, 1, 0))
    for (i in 1:number_of_plots) {
    plot(date ~ interval, named_plots[[i]],
         xlab = "", ylab = "", main = names(named_plots)[i], cex.main = 0.7,
         col = colorscale(steps),
...
However the `mean` seems to have shifted from `9354` to `9705`.
